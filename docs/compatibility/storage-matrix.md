...
| Artifact | 参考 `GenerationArtifact`，支持类型和内容读取；写入在 `harness/orchestrator._save_checkpoint()` 里经 `session_store.save_artifact()` 落 BOS + 表 | `generation_artifact`（Step 6 新增，`tables.py:137`），`metadata` 列以属性名 `meta` 映射（`metadata` 是声明式保留名）；Step 7 起有写入点 | 已有写入点：`harness/orchestrator._save_artifact()` 写 `analysis`/`design`/`verification` 三类，version 按 (session_id, artifact_type) 递增，`meta={"format": ...}`；`bos_path` 仍为空、`code`/`fix` 类型无写入点，待 Step 9 |
| Harness checkpoint | 参考另有 `harness_checkpoint` 表 + Redis 双层 checkpoint（`page/harness/checkpoint.py`） | 无该表，checkpoint 仍由 LangGraph `MemorySaver` 承担 | 未对齐（Step 7 已知差异）：进程重启后图内 checkpoint 丢失，恢复依赖 Job 重跑；Step 9 统一收口 |
...
- 产物：`harness/orchestrator._save_artifact()` 每次节点产出后插入一行 `generation_artifact`（内联实现，Step 9 迁到 `services/artifact_store.py`）。落库失败只记日志，不打断流水线。
...
- `attachment`、`skill_definition`、`page_template` 只建表，无写入点（Step 9/11/12）；`generation_artifact` 已有写入点，但只覆盖 analysis/design/verification 三类；
- 参考另有 `harness_checkpoint` 表（MySQL + Redis 双层 checkpoint），sinan 无该表，checkpoint 仍由 LangGraph `MemorySaver` 承担 —— Step 7 的已知差异，Step 9 统一收口；
- `harness/validators/browser_validator.py` 自 Step 7 起无引用点（原由 `agents/verifier.py` 调用），渲染校验已收敛到 `harness/validators/render.py`；文件保留待 Step 13 复用或删除；
...
- `next_version()` 与 `save()` 非原子，同 marker 并发生成会撞 `uk_marker_version`（Step 6 之前既有行为）；`_save_artifact()` 的 `max(version)+1` 同理非原子，同 session 同类型并发会撞 `uk_artifact_session_type_version`（当前流水线内串行，暂不加锁）；
