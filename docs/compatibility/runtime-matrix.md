...
| Runtime 选择 | `mode`、配置及 Claude Code/Opencode/Hybrid 分支 | `RuntimeRegistry.get(mode)` 按 mode 路由（Step 13）；`native` 和 `claude_code` 已接入，底层均为 `NativeLangGraphRuntime`；`opencode`/`hybrid` 待实现（Step 14） | 部分对齐：native/claude_code mode 已路由；opencode 分支缺失（Step 14） |
...
| 默认生成图节点数 | 6 节点 | **7 节点（Step 12）**：`router → skill → analyst → designer → coder → verifier → fixer`，`skill` 不套 `wrap_node` | 未对齐（sinan 增量）：参考把 Skill 预处理放在 runtime 层（`claude_code/codegen_engine.preprocess_skill`）而非图节点。Step 13 引入 Runtime 抽象后，skill 节点仍保留在 NativeLangGraphRuntime 的图中；ClaudeCodeRuntime 当前底层也是 native 图，暂不下移，Step 14 对照时再评估 |
